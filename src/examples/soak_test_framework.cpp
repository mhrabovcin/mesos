...
// Actor holding the business logic and metrics for the `SoakTestScheduler`.
// See `SoakTestScheduler` below for intended behavior.
class SoakTestSchedulerProcess
  : public process::Process<SoakTestSchedulerProcess>
{
public:
  SoakTestSchedulerProcess (
      const Flags& _flags,
...
private:
  ...
  struct Metrics
  {
    Metrics(const SoakTestSchedulerProcess& _scheduler)
      : uptime_secs(
            "soak_test_framework/uptime_secs",
            defer(_scheduler, &SoakTestSchedulerProcess::_uptime_secs)),
        registered(
            "soak_test_framework/registered",
            defer(_scheduler, &SoakTestSchedulerProcess::_registered)),
        tasks_finished(
            "soak_test_framework/tasks_finished"),
        abnormal_terminations(
            "soak_test_framework/abnormal_terminations")
    {
...
class SoakTestScheduler : public Scheduler
{
public:
  SoakTestScheduler(const Flags& _flags, const FrameworkInfo& _frameworkInfo)
    : process(_flags, _frameworkInfo)
  {
    process::spawn(process);
  }

  virtual ~SoakTestScheduler()
  {
      ...
      const MasterInfo&)
  {
    ...
    process::dispatch(&process, &SoakTestSchedulerProcess::registered);
  }
  ...
  virtual void disconnected(SchedulerDriver*)
  {
    ...
    process::dispatch(
        &process,
        &SoakTestSchedulerProcess::disconnected);
  }
      ...
      const std::vector<Offer>& offers)
  {
    ...
    process::dispatch(
         &process,
         &SoakTestSchedulerProcess::resourceOffers,
         driver,
         offers);
  }
  ...
  virtual void statusUpdate(SchedulerDriver* driver, const TaskStatus& status)
  {
    ...
    process::dispatch(
        &process,
        &SoakTestSchedulerProcess::statusUpdate,
        driver,
        status);
  }
...
private:
  SoakTestSchedulerProcess process;
};
...
int main(int argc, char** argv)
{
  ...
  framework.set_name("Soak Test Framework (C++)");
  framework.set_checkpoint(true);

  SoakTestScheduler scheduler(flags, framework);
  ...
  if (value.isSome()) {
    ...
    framework.set_principal("soak-test-framework-cpp");
